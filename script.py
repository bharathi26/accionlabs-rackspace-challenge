...
def transform_data():
    ...
    try:
        for x in my_list:
            tag_count = len(x.partition(' ')[0]) + 1
            if x.startswith("* "):
                h2_count = 1
                h1 = etree.SubElement(root, "h1")
                h1.set("text_val", x[tag_count:])
